 ...
 * 只有当 队头邮件优先级值 <= 请求优先级值 (priority) 时, 才允许取出.
 */
@Slf4j
public class TaskMailboxImpl implements TaskMailbox {
    ...
    public Optional<Mail> tryTake(int priority) {
        ...
        try {
            ...
            // 2. [关键模仿 Flink] 优先级不满足
            // 如果 head.priority (比如 1) > required priority (比如0)
            // 说明虽有信, 但这封信不够格, 不能在这里被取出
    ...
    public Mail take(int priority) throws InterruptedException {
        ...
        try {
            ...
            while (isQueueEmptyPriorityTooLow(priority)) {
     ...
     * 辅助判断逻辑: 否是需要阻塞
     */
    private boolean isQueueEmptyPriorityTooLow(int priority) {
        Mail head = queue.peek();
        if (head == null) {
            return true; // 空, 需要等
        }
