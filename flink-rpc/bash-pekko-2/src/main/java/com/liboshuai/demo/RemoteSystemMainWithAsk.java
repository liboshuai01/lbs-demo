...
public class RemoteSystemMainWithAsk {
    ...
    public static void main(String[] args) {
        ...
        log.info("远程系统 'ask' 演示已准备就绪。"); // RemoteSystem for 'ask' demo is ready.
        ...
        log.info("已创建远程问候 Actor 并支持回复。完整路径: {}", remoteGreeter.path()); // Remote Greeter actor with reply created. Full path: {}

        log.info(">>> 按 ENTER 退出 <<<"); // >>> Press ENTER to exit <<<
        try {
            System.in.read();
        } catch (Exception e) {
            e.printStackTrace();
